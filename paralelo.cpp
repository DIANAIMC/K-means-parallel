...
/*
    ...
    max_iterations
*/
void kmeans_paralelo(float** points, int n_clusters, long long int size, int max_iterations) {
    // Initialize cluster assignments randomly
    // 1
    #pragma omp parallel for // Este es el que está bajando el tiempo
    ...
    while (!all_stable && iteration < max_iterations) {
        ...
        // Inicializamos los centroides de los clusters
        //#pragma omp parallel for ----->>> Este no funciona, por qué?
        ...
        //#pragma omp parallel for reduction(+:centroids[:n_clusters][:2])
        //#pragma omp critical ---> Este tampoco baja el tiempo
        for (long long int i=0; i < size; i++) {
        ...
        #pragma omp parallel for reduction(&&:all_stable) // Baja bastante el tiempo
        for (long long int i=0; i < size; i++) {
...
int main(int argc, char** argv) {
    ...
    delete[] paralelo;
    return tiempo_ejecucion_paralelo;
}